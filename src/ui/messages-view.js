...
export function messagesView(character, state) {
  ...
  const chat = current ? '<div class="chat-date">今天 · 雾港时间</div><div class="chat-message incoming"><span class="message-avatar">沈</span><div><div class="bubble">' + escapeHtml(current.text) + '</div><small>' + current.time + '</small></div></div>' +
    replies.map((reply) => '<div class="chat-message outgoing"><div><div class="bubble">' + escapeHtml(reply.text) + '</div><small>你 · 已送达</small></div></div><div class="chat-message incoming"><span class="message-avatar">沈</span><div><div class="bubble">' + escapeHtml(reply.response) + '</div><small>沈砚舟 · 已读</small></div></div>').join("") +
  ...
  return '<section class="page subpage messages-page"><header class="subpage-header"><div><div class="eyebrow">PERSONAL LINE · 01</div><h1>通讯</h1><p>不必时刻在线，他会记得回来找你。</p></div><span class="secure-tag">◈ 私人频道</span></header>' +
    '<div class="messages-layout"><aside class="inbox-panel panel-card"><div class="inbox-heading"><span>最近联系</span><span>01</span></div>' +
      '<button class="inbox-contact active"><span class="message-avatar large">沈</span><span class="contact-copy"><b>' + character.name + '</b><small>' + (state.sceneCompleted ? "新约定 · 潮汐站" : "调查结束了吗？") + '</small></span><span class="contact-time">20:16<i></i></span></button>' +
      '<div class="inbox-note"><span class="note-mark">✳</span><p>有些消息没有期限。<br/>有些人会记得你说过的话。</p></div></aside>' +
      '<section class="chat-panel panel-card"><header class="chat-header"><span class="message-avatar">沈</span><div><b>' + character.name + '</b><small><i></i> 正在雾港 · 休息室</small></div><button class="icon-button chat-more" data-route="profile">···</button></header>' +
